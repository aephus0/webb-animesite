...
import { Anime } from "../moodles/anime.js";
...
router.get("/:id", async (req, res) => {
    ...
    try {
        ...
        return res.json(new ErrorRes("Error", "Something went horribly wrong, try again later"));
    }
});
...
router.post("/", validateBearer, newanimeitem, async (req, res) => {
    try {
        ...
        return res.json(new ErrorRes("Error", 'Something went horribly wrong while processing the JSON data, did you forget the ""?'));
    }
    ...
    try {
        ...
        if (anime !== null) {
            return res.json(new FailRes("Fail", { title: "The title already exists" }));
        }
        var id = generate(5);
        ...
        res.json(new ErrorRes("Error", "Something went horribly wrong, try again later"));
    }
});
...
router.put("/", validateBearer, updateanime, async (req, res) => {
    ...
    try {
        ...
        const result = await Anime.updateOne({ aniId: req.body.aniId }, { title: newani.title, description: newani.description });
        return res.json(new SuccessRes("Success", result));
    }
    catch (err) {
